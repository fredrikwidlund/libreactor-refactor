...
{
  return buffer_at(&v->buffer, position * v->object_size);
}

void *vector_front(vector *v)
{
  return vector_base(v);
}

void *vector_back(vector *v)
{
  return buffer_at(&v->buffer, buffer_size(&v->buffer) - v->object_size);
}

void *vector_base(vector *v)
{
  return buffer_base(&v->buffer);
}
